...
class DataGetter:
    def __init__(self, videos_dir, cache_dir=None):
        self.videos_dir = videos_dir
        self.cache_dir = cache_dir

    def get_cache(self):
        if self.cache_dir:
            y_file = self.cache_dir / "y.pkl"
            misc_features_file = self.cache_dir / "misc_features.pkl"
            target_ts_file = self.cache_dir / "poses_ts.pkl"

            if (
                y_file.exists()
                and misc_features_file.exists()
                and target_ts_file.exists()
            ):
    ...
    def save_cache(self, all_y, all_file_features, all_target_ts):
    ...
    def get_input_data(self):
        all_y, all_misc_features, all_target_ts = self.get_cache()

        video_paths = set(self.videos_dir.iterdir())
        ...
        if filtered:
            ...
            self.save_cache(all_y, all_misc_features, all_target_ts)
